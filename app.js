...
(function() {
  ...
  var adminKey = ''; // stored after successful login for API auth
  ...
  function setAdminAuthenticated(pw) {
    ...
    return subs.map(function(sub) {
      ...
      return '<div class="admin-subscriber-item">' +
        ...
        '<div class="admin-subscriber-item__meta">' +
          ...
          (sub.status === 'pending' ? '<button type="button" class="admin-subscriber-approve" data-sub-id="' + sub.id + '" data-sub-email="' + escapeHTML(sub.email) + '" title="Approve subscriber">Approve</button>' : '') +
  ...
  function bindRemoveButtons(container) {
    ...
    // Approve buttons for pending subscribers
    container.querySelectorAll('.admin-subscriber-approve').forEach(function(btn) {
      btn.addEventListener('click', function() {
        var subId = btn.getAttribute('data-sub-id');
        var subEmail = btn.getAttribute('data-sub-email');
        if (!confirm('Approve "' + subEmail + '"? They will be marked as active and receive newsletters.')) return;
        btn.disabled = true;
        btn.textContent = '...';
        fetch('/api/subscribers?id=' + encodeURIComponent(subId), { method: 'PUT' })
          .then(function(r) { return r.json(); })
          .then(function(res) {
            if (res.success) {
              renderSubscriberList();
  ...
  function renderTrainingCourses() {
    ...
    // ── TrainIQ: "Your Learning Path" personalised section ──────────────────────
    (function() {
      try {
        var saved = localStorage.getItem('trainiq_learning_path');
        if (!saved) return;
        var lp = JSON.parse(saved);
        var ageDays = (Date.now() - new Date(lp.date).getTime()) / 86400000;
        if (ageDays > 90 || !lp.courses || !lp.courses.length) return;
        ...
        if (!document.getElementById('lp-styles')) {
          ...
          style.id = 'lp-styles';
          style.textContent =
            '.lp-section{margin-bottom:2.5rem;padding:1.25rem;background:var(--color-surface,#1e293b);border:1px solid rgba(45,212,191,0.18);border-radius:12px}' +
            '.lp-header{display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:.5rem;margin-bottom:1rem}' +
            '.lp-title{display:flex;align-items:center;gap:.5rem;font-weight:600;font-size:.9rem;color:#e2e8f0}' +
            '.lp-badge{font-size:.7rem;font-weight:600;padding:.15rem .55rem;border-radius:999px}' +
            '.lp-actions{display:flex;align-items:center;gap:.75rem}' +
            '.lp-retake{font-size:.8rem;color:#2dd4bf;text-decoration:none;white-space:nowrap}' +
            '.lp-retake:hover{text-decoration:underline}' +
            '.lp-dismiss{background:none;border:none;color:#64748b;cursor:pointer;font-size:1.1rem;padding:.1rem .3rem;line-height:1}' +
            '.lp-dismiss:hover{color:#94a3b8}' +
            '.lp-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:.875rem}';
          document.head.appendChild(style);
        }

        // Create or reuse container — insert before the first course section
        var anchor = freeContainer || paidContainer;
        var parentEl = anchor.parentNode;
        var section = parentEl.querySelector('.lp-section');
        if (!section) {
          section = document.createElement('div');
          section.className = 'lp-section reveal';
          parentEl.insertBefore(section, anchor);
        }

        var classColor = lp.classification === 'Advanced' ? '#f59e0b' :
                         lp.classification === 'Informed'  ? '#2dd4bf' : '#60a5fa';

        var coursesHTML = lp.courses.map(function(c) {
          return '<a href="' + escapeHTML(c.enroll_url || '#') + '" target="_blank" rel="noopener noreferrer" ' +
            'class="course-card" style="text-decoration:none;display:block">' +
            '<div class="course-card__body">' +
              '<div class="course-card__provider">' + escapeHTML(c.provider || '') + '</div>' +
              '<h4 class="course-card__title" style="font-size:.875rem">' + escapeHTML(c.title) + '</h4>' +
              '<div class="course-card__details">' +
                '<span class="course-card__detail">' + escapeHTML(c.level || '') + '</span>' +
              '</div>' +
            '</div>' +
            '<div class="course-card__footer">' +
              '<span class="course-card__price">' + (c.is_free ? 'Free' : 'Paid') + '</span>' +
              '<span class="course-card__link">Enroll →</span>' +
            '</div>' +
          '</a>';
        }).join('');
        ...
        section.innerHTML =
          '<div class="lp-header">' +
            '<div class="lp-title">' +
              '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="' + classColor + '" stroke-width="2.5" stroke-linecap="round">' +
                '<path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/>' +
              '</svg>' +
              'Your Learning Path' +
              '<span class="lp-badge" style="background:' + classColor + '22;color:' + classColor + ';border:1px solid ' + classColor + '44">' +
                escapeHTML(lp.classification || '') +
              '</span>' +
            '</div>' +
            '<div class="lp-actions">' +
              '<a href="./train-iq.html" class="lp-retake">Retake TrainIQ →</a>' +
              '<button class="lp-dismiss" title="Dismiss" ' +
                'onclick="localStorage.removeItem('trainiq_learning_path');this.closest('.lp-section').remove()">&#x2715;</button>' +
            '</div>' +
          '</div>' +
          '<div class="lp-grid">' + coursesHTML + '</div>';
  ...
  // Meet IQ featured tool SVG logo
  var meetIQLogo = '<svg class="tool-featured__logo" width="40" height="40" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg"><rect width="32" height="32" rx="8" fill="rgba(0,196,204,0.1)" stroke="rgba(0,196,204,0.2)" stroke-width="1"/><path d="M7 16 Q10.5 10 14 16 Q17.5 22 21 16 Q23.5 11.5 25 16" stroke="#00C4CC" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" fill="none"/><circle cx="16" cy="16" r="2.5" fill="rgba(0,196,204,0.3)" stroke="#00C4CC" stroke-width="1.25"/></svg>';

  // Train IQ logo — checklist + badge in teal
  var trainIQLogo = '<svg class="tool-featured__logo" width="40" height="40" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg"><rect width="32" height="32" rx="8" fill="rgba(45,212,191,0.1)" stroke="rgba(45,212,191,0.2)" stroke-width="1"/><rect x="8" y="7" width="11" height="14" rx="2" stroke="#2dd4bf" stroke-width="1.4" fill="none"/><line x1="11" y1="11" x2="16" y2="11" stroke="#2dd4bf" stroke-width="1.2" stroke-linecap="round"/><line x1="11" y1="14" x2="16" y2="14" stroke="#2dd4bf" stroke-width="1.2" stroke-linecap="round"/><line x1="11" y1="17" x2="14" y2="17" stroke="#2dd4bf" stroke-width="1.2" stroke-linecap="round"/><circle cx="21" cy="21" r="5" fill="rgba(45,212,191,0.15)" stroke="#2dd4bf" stroke-width="1.4"/><path d="M18.5 21l1.5 1.5 2.5-2.5" stroke="#2dd4bf" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/></svg>';
  ...
  function buildFeaturedToolHTML(tool) {
    ...
    var actionsHTML = '';
    if (tool.launch_url) {
      var launchHref = escapeHTML(tool.launch_url) + (tool.launch_url.indexOf('?') >= 0 ? '&' : '?') + 'v=' + Date.now();
      actionsHTML += '<a href="' + launchHref + '" target="_blank" rel="noopener noreferrer" class="tool-featured__btn tool-featured__btn--primary">' +
    ...
    if (tool.mobile_url) {
      actionsHTML += '<a href="' + escapeHTML(tool.mobile_url) + '" target="_blank" rel="noopener noreferrer" class="tool-featured__btn tool-featured__btn--secondary">' +
        '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg> User Manual</a>';
    }

    var toolLogo = (tool.name && tool.name.indexOf('Train') >= 0) ? trainIQLogo : meetIQLogo;
    return '<div class="tool-featured reveal">' +
      '<div class="tool-featured__header">' + toolLogo +
        '<div><h3 class="tool-featured__name">' + escapeHTML(tool.name).replace(/Meet IQ/g, 'Meet<sup style="font-size:0.55em">IQ</sup>').replace(/Train IQ/g, 'Train<sup style="font-size:0.55em">IQ</sup>') + '</h3>' +
        '<p class="tool-featured__tagline">' + escapeHTML(tool.tagline || '') + '</p></div>' +
        '<span class="badge badge--live">Live</span>' +
      '</div>' +
      '<p class="tool-featured__desc">' + escapeHTML(tool.description || '').replace(/Meet IQ/g, 'Meet<sup style="font-size:0.55em">IQ<\/sup>') + '</p>' +
        ...
        if (comingTools.length > 0) {
          ...
          upcomingContainer.innerHTML = comingTools.map(function(tool) {
            ...
            return '<article class="tool-card tool-card--teaser">' +
              ...
              '<h3 class="tool-card__title">' + escapeHTML(tool.name).replace(/Meet IQ/g, 'Meet<sup style="font-size:0.55em">IQ</sup>').replace(/Train IQ/g, 'Train<sup style="font-size:0.55em">IQ</sup>') + '</h3>' +
              '<p class="tool-card__desc">' + escapeHTML(tool.description || '').replace(/Meet IQ/g, 'Meet<sup style="font-size:0.55em">IQ<\/sup>') + '</p>' +
  ...
  var tableFields = {
    ...
    ai_tools: [
      { key: 'name', label: 'Tool Name', type: 'text', placeholder: 'e.g. Meet IQ', required: true },
      ...
      { key: 'launch_url', label: 'Web Launch URL', type: 'text', placeholder: './meet-iq-pro.html' },
      { key: 'mobile_url', label: 'Mobile Launch URL', type: 'text', placeholder: './meet-iq-guide.html' },
  ...
  if (adminTabsContainer) {
    ...
    allAdminTabBtns.forEach(function(tabBtn) {
      tabBtn.addEventListener('click', function() {
        ...
        } else if (target === 'mp-users') {
          showAdminView('mp-users');
          renderMPUsers();
        } else if (target === 'mp-feedback') {
          showAdminView('mp-feedback');
          renderMPFeedback();
  ...
  // ========================================
  // ========================================
  // MEET IQ USERS ADMIN VIEW
  // ========================================

  function renderMPUsers() {
    var statsEl = document.getElementById('mp-user-stats');
    var listEl = document.getElementById('mp-user-list');
    if (!listEl) return;

    listEl.innerHTML = '<p class="admin-empty">Loading Meet IQ users...</p>';

    fetch('/api/content?table=meetpilot_user_profiles')
      .then(function(r) { return r.json(); })
      .then(function(data) {
        if (!Array.isArray(data) || data.length === 0) {
          if (statsEl) statsEl.innerHTML = '<div class="stat"><span class="stat-number">0</span> users</div>';
          listEl.innerHTML = '<p class="admin-empty">No Meet IQ users yet. Users will appear here after signing up via the auth gate.</p>';
          return;
        }

        var proCount = data.filter(function(u) { return u.edition === 'pro' || u.edition === 'both'; }).length;
        var freeCount = data.filter(function(u) { return u.edition === 'free'; }).length;

        if (statsEl) {
          statsEl.innerHTML =
            '<div class="stat"><span class="stat-number">' + data.length + '</span> total</div>' +
            '<div class="stat"><span class="stat-number active">' + proCount + '</span> pro</div>' +
            '<div class="stat"><span class="stat-number">' + freeCount + '</span> free</div>';
        }

        listEl.innerHTML = data.map(function(user) {
          var editionBadge = user.edition === 'pro' ? '<span style="background:#059669;color:#fff;padding:1px 6px;border-radius:4px;font-size:0.65rem;font-weight:700">PRO</span>'
            : user.edition === 'both' ? '<span style="background:#8B5CF6;color:#fff;padding:1px 6px;border-radius:4px;font-size:0.65rem;font-weight:700">BOTH</span>'
            : '<span style="background:#3B82F6;color:#fff;padding:1px 6px;border-radius:4px;font-size:0.65rem;font-weight:700">FREE</span>';
          var created = user.created_at ? new Date(user.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '—';
          var lastLogin = user.last_login_at ? new Date(user.last_login_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : '—';
          var company = user.company ? ' <span style="color:#8A8A8E">(' + user.company + ')</span>' : '';

          return '<div class="subscriber-item" style="display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:1px solid #252530">' +
            '<div style="flex:1;min-width:0">' +
              '<div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">' +
                '<strong style="font-size:0.8rem">' + (user.full_name || 'Anonymous') + '</strong> ' + editionBadge + company +
              '</div>' +
              '<div style="font-size:0.7rem;color:#8A8A8E;margin-top:2px">' + (user.email || '') + (user.role ? ' · ' + user.role : '') + '</div>' +
            '</div>' +
            '<div style="text-align:right;flex-shrink:0;font-size:0.65rem;color:#8A8A8E">' +
              '<div>Joined ' + created + '</div>' +
              '<div>Last: ' + lastLogin + '</div>' +
            '</div>' +
            '<div style="flex-shrink:0;display:flex;gap:4px;align-items:center">' +
              '<select class="admin-input" style="font-size:0.65rem;padding:2px 6px;width:70px" data-mp-user-id="' + user.id + '" data-mp-edition>' +
                '<option value="free"' + (user.edition === 'free' ? ' selected' : '') + '>Lite</option>' +
                '<option value="pro"' + (user.edition === 'pro' ? ' selected' : '') + '>Pro</option>' +
                '<option value="both"' + (user.edition === 'both' ? ' selected' : '') + '>Both</option>' +
              '</select>' +
              '<button class="admin-btn admin-btn--sm" style="font-size:0.6rem;padding:3px 6px;white-space:nowrap" data-mp-reset="' + (user.email || '') + '" title="Send password reset email">Reset PW</button>' +
              '<button class="admin-btn admin-btn--sm" style="font-size:0.6rem;padding:3px 6px;color:#EF4444;border-color:#EF4444" data-mp-delete="' + user.id + '" data-mp-delete-name="' + (user.full_name || user.email || '') + '" title="Delete user">✕</button>' +
            '</div>' +
          '</div>';
        }).join('');

        // Bind edition change handlers
        listEl.querySelectorAll('[data-mp-edition]').forEach(function(sel) {
          sel.addEventListener('change', function() {
            var userId = sel.getAttribute('data-mp-user-id');
            var newEdition = sel.value;
            fetch('/api/content?table=meetpilot_user_profiles&id=' + userId, {
              method: 'PATCH',
              headers: getAdminHeaders(),
        ...
        // Bind reset password handlers
        listEl.querySelectorAll('[data-mp-reset]').forEach(function(btn) {
          btn.addEventListener('click', function() {
            var email = btn.getAttribute('data-mp-reset');
            ...
            fetch('/api/content', {
              ...
              btn.textContent = d.success ? 'Sent!' : 'Error';
              btn.style.color = d.success ? '#22C55E' : '#EF4444';
              setTimeout(function() { btn.textContent = 'Reset PW'; btn.style.color = ''; }, 2500);
            }).catch(function() {
              btn.textContent = 'Error';
              setTimeout(function() { btn.textContent = 'Reset PW'; }, 2000);
            });
          });
        });

        // Bind delete handlers
        listEl.querySelectorAll('[data-mp-delete]').forEach(function(btn) {
          btn.addEventListener('click', function() {
            var userId = btn.getAttribute('data-mp-delete');
            var userName = btn.getAttribute('data-mp-delete-name');
            if (!confirm('Delete user "' + userName + '"? This cannot be undone.')) return;
            btn.textContent = '...';
            fetch('/api/content', {
              method: 'POST',
              headers: getAdminHeaders(),
              body: JSON.stringify({ action: 'delete_mp_user', user_id: userId })
            }).then(function(r) { return r.json(); }).then(function(d) {
              if (d.success) {
                btn.closest('.subscriber-item').style.opacity = '0.3';
                btn.closest('.subscriber-item').style.pointerEvents = 'none';
                setTimeout(function() { renderMPUsers(); }, 1000);
              } else {
      ...
      .catch(function(err) {
        listEl.innerHTML = '<p class="admin-empty">Error loading users: ' + err.message + '</p>';
      });
  }

  // ========================================
  // MEET IQ FEEDBACK ADMIN VIEW
  // ========================================

  function renderMPFeedback() {
    var statsEl = document.getElementById('mp-feedback-stats');
    var listEl = document.getElementById('mp-feedback-list');
    if (!listEl) return;

    listEl.innerHTML = '<p class="admin-empty">Loading feedback...</p>';

    fetch('/api/content?table=meetpilot_session_feedback')
      .then(function(r) { return r.json(); })
      .then(function(data) {
        if (!Array.isArray(data) || data.length === 0) {
          if (statsEl) statsEl.innerHTML = '<div class="stat"><span class="stat-number">0</span> reviews</div>';
          listEl.innerHTML = '<p class="admin-empty">No feedback yet. Feedback will appear here after users rate their sessions.</p>';
          return;
        }
        ...
        var avgRating = data.reduce(function(sum, f) { return sum + (f.rating || 0); }, 0) / data.length;
        var proCount = data.filter(function(f) { return f.edition === 'pro'; }).length;
        var freeCount = data.filter(function(f) { return f.edition === 'free'; }).length;

        if (statsEl) {
          statsEl.innerHTML =
            '<div class="stat"><span class="stat-number">' + data.length + '</span> reviews</div>' +
            '<div class="stat"><span class="stat-number active">' + avgRating.toFixed(1) + '</span> avg rating</div>' +
            '<div class="stat"><span class="stat-number">' + proCount + '</span> pro</div>' +
            '<div class="stat"><span class="stat-number">' + freeCount + '</span> free</div>';
        }

        listEl.innerHTML = data.map(function(fb) {
          var stars = '';
          for (var i = 0; i < 5; i++) stars += (i < fb.rating) ? '\u2605' : '\u2606';
          var editionBadge = fb.edition === 'pro'
            ? '<span style="background:#059669;color:#fff;padding:1px 6px;border-radius:4px;font-size:0.6rem;font-weight:700">PRO</span>'
            : '<span style="background:#3B82F6;color:#fff;padding:1px 6px;border-radius:4px;font-size:0.6rem;font-weight:700">FREE</span>';
          var created = fb.created_at ? new Date(fb.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric', hour: 'numeric', minute: '2-digit' }) : '\u2014';
          var dur = fb.duration_seconds ? Math.round(fb.duration_seconds / 60) + ' min' : '\u2014';
          var words = fb.total_words ? fb.total_words.toLocaleString() + ' words' : '';
          var comment = fb.comment ? '<div style="margin-top:4px;font-size:0.75rem;color:#C8C8CC;background:#1C1C1E;padding:6px 8px;border-radius:4px;border-left:2px solid #4FFFB0">' + fb.comment.replace(/</g, '&lt;').replace(/>/g, '&gt;') + '</div>' : '';
          var userName = fb.user_name ? fb.user_name : 'Anonymous';
          var userEmail = fb.user_email ? ' <span style="color:#8A8A8E">(' + fb.user_email + ')</span>' : '';

          return '<div class="subscriber-item" style="padding:10px 0;border-bottom:1px solid #252530">' +
            '<div style="display:flex;align-items:center;gap:8px;margin-bottom:4px">' +
              '<span style="color:#FFD700;font-size:0.9rem;letter-spacing:1px">' + stars + '</span> ' +
              editionBadge +
              '<strong style="font-size:0.78rem">' + userName + '</strong>' + userEmail +
              '<span style="margin-left:auto;font-size:0.65rem;color:#8A8A8E">' + created + '</span>' +
            '</div>' +
            '<div style="font-size:0.68rem;color:#8A8A8E">' +
              dur + (words ? ' \u00b7 ' + words : '') +
              ' \u00b7 ' + (fb.meeting_type || 'general') +
              ' \u00b7 AI: ' + (fb.suggestion_model || '\u2014') +
            '</div>' +
            comment +
          '</div>';
        }).join('');
      ...
      .catch(function(err) {
        listEl.innerHTML = '<p class="admin-empty">Error loading feedback: ' + err.message + '</p>';
      });
  }
  ...
  (function handleDeepLink() {
    var hash = window.location.hash;
    if (!hash) return;
    // Longer delay for sections that load from Supabase (tools, etc.)
    var delay = (hash === '#tools' || hash === '#training' || hash === '#investments') ? 1500 : 600;
    setTimeout(function() {
